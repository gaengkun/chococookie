...
const userMenu = useRef();
 ...
 function useInterval(callback, delay) {
  ...
  const modalCloseHandler = ({ target }) => {
    console.log(`열림`, menuType!='')
    console.log(menuType!='' &&userMenu.current.contains(target))
    // console.log(menuType!='' && userMenu.current.contains(target))
    // console.log(`전체`, target)

    if (menuType!='' &&userMenu.current.contains(target)){
      setMenuType('');
    }
  };
  ...
  <div className="wrap h-full" >
      ...
      <div className={`absolute w-full h-full mask duration-[1000ms] z-20 ${mainContent == true ? 'opacity-20' : 'opacity-100'}`}></div>
      {BOX_LIST.map(item => {
        const visualClass = ['mask','space', 'mounten', 'space', 'winter']
        return<div
            key={item.id}
            className={`absolute w-full h-full ${visualClass[item.id]} ${visualCount === item.id ? 'active z-10 opacity-100 ease-in duration-[1200ms]' : ' ease-out duration-1000 opacity-0 z-0'}`}
      ...
      <div className="flex min-w-320 flex-col items-center justify-center absolute z-30 left-0 top-0 w-full h-full text-white p-6">
          ...
          <div className="border-y w-full max-w-2xl">
            <div className={`overflow-hidden ease-out duration-500 ${mainContent == true ? 'py-20 min-h-min opacity-100' : 'h-0 opacity-0'} sm:py-12 `}>
              <h2 className={`text-center delay-300 ease-in duration-300 text-6xl font-semibold pb-10 tracking-widest ${mainContent == true ? 'min-h-min opacity-100' : 'h-0 opacity-0'} sm:text-4xl`}>DIMENSION</h2>
              <p className={`text-center delay-300 ease-in duration-300 font-thin tracking-widest leading-8 ${mainContent == true ? 'min-h-min opacity-100' : 'h-0 opacity-0'} sm:text-sm sm:leading-8`}>
      ...
      {menuType != '' ?<div ref={userMenu}><MainPopupLayer type={menuType} isShow={setMenuType}/></div>:'' }
